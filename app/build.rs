...
{}",
    ...
    // ===== VALIDATE MANIFEST HAS MEANINGFUL DATA =====
    // Detect the placeholder/empty case early with a clear error message,
    // instead of producing a broken demo binary that crashes at runtime.
    let manifest_len = manifest_str.len();
    let manifest_json: serde_json::Value =
        serde_json::from_str(&manifest_str).expect("Manifest already validated");
    let images_count = manifest_json["project"]["images"]
        .as_array()
        .map(|a| a.len())
        .unwrap_or(0);
    let outputs_count = manifest_json["project"]["outputs"]
        .as_array()
        .map(|a| a.len())
        .unwrap_or(0);
    let projects_count = manifest_json["projects"]
        .as_array()
        .map(|a| a.len())
        .unwrap_or(0);

    // A real manifest (generated by test_generate_demo_data) is hundreds of KB
    // with dozens of images and outputs. Anything below 2KB or with empty
    // images/outputs arrays is almost certainly the placeholder.
    if manifest_len < 2000 || images_count == 0 || outputs_count == 0 {
        panic!(
            "Generated demo manifest is too small or has empty fields:\n\
             {} bytes, {} images, {} outputs, {} projects.\n\
             This means the demo data has not been generated.\n\
             The demo data is NOT committed to the repository (see .gitignore).\n\
             Complete manifest:\n{:#}\n\
             Generate it by running:\n\
             $ cargo test --test generate_demo_data -p colmap-openmvs-backend\n\
             This will populate the `app/assets/demo/` directory with the\n\
             manifest.json, images, and outputs needed for a working demo.\n\
             In CI, this test runs automatically before the web-demo build.",
            manifest_len, images_count, outputs_count, projects_count, manifest_json,
        );
    }
